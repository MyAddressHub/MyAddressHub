...
export const PermissionManager: React.FC<PermissionManagerProps> = ({
    ...
    const loadCurrentPermissions = async () => {
        ...
        try {
            const response = await addressesAPI.getAddressPermissions(
                address.id
            )
            ...
            if (response.success) {
                ...
                console.log(
                    "Failed to load current permissions:",
                    response.error
                )
    ...
    const handleGrantPermission = async () => {
        ...
        try {
            const response = await addressesAPI.grantAddressPermission(
                address.id,
            ...
            setError(
                error.response?.data?.error || "Failed to grant permission"
            )
        } finally {
            setIsLoading(false)
        }
    ...
    const handleRevokePermission = async (
        ...
        console.log("handleRevokePermission called with:", {
            organizationId,
            organizationName,
        })
        ...
        if (
            !confirm(
                `Are you sure you want to revoke permission from ${organizationName}?`
            )
        ) {
        ...
        try {
            ...
            const response = await addressesAPI.revokeAddressPermission(
                address.id,
                organizationId
            )
            ...
            setError(
                error.response?.data?.error || "Failed to revoke permission"
            )
        } finally {
            setIsLoading(false)
        }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <div className="mb-6">
                    <h4 className="text-md font-medium text-stone-950 dark:text-white mb-3">
                ...
                <div>
                    ...
                    {currentPermissions.length === 0 ? (
                        <p className="text-gray-500 dark:text-gray-400 text-sm">
                            No organizations currently have access to this
                            address.
                        </p>
                    ) : (
                        <div className="space-y-2">
                            {currentPermissions.map((permission) => (
                                ...
                                >
                                    <div>
                                        <h5 className="font-medium text-stone-950 dark:text-white">
                                        ...
                                        <p className="text-xs text-gray-400 dark:text-gray-500">
                                            Granted by{" "}
                                            {permission.grantedByUsername} on{" "}
                                            {new Date(
