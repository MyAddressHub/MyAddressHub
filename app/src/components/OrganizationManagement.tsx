...
const OrganizationManagement: React.FC = () => {
  ...
  const fetchOrganizationUsers = async () => {
    try {
      ...
      if (response.data && typeof response.data === 'object' && !Array.isArray(response.data)) {
        ...
        membersData = response.data.results || response.data.data || response.data.members || [];
      }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('/api/auth/organization/users/create/', createForm);
  ...
  const handleUpdateUserRole = async (memberId: string, newRole: string) => {
    ...
    try {
      const response = await axios.patch(`/api/auth/organization/users/${memberId}/role/`, { role: newRole });
  ...
  const handleDeactivateUser = async (memberId: string) => {
    ...
    try {
      const response = await axios.patch(`/api/auth/organization/users/${memberId}/deactivate/`, { is_active: false });
  ...
  const getRoleBadgeColor = (role: string) => {
    switch (role) {
      case 'owner': return 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200';
      case 'admin': return 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-200';
      case 'manager': return 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200';
      case 'member': return 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200';
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
          <h2 className="text-lg font-semibold text-yellow-800 dark:text-yellow-200">Organization User Management</h2>
          <p className="text-yellow-700 dark:text-yellow-300 mt-2">
            This feature is only available for organization users. Please contact your administrator to join an organization.
          </p>
        </div>
        </div>
      </div>
                  ...
                  <tr key={member.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400">@{member.userUsername}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${member.isActive ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      <select
                        ...
                        disabled={member.userUsername === user?.username}
                      >
                      ...
                      {member.userUsername !== user?.username && member.isActive && (
                        <button
