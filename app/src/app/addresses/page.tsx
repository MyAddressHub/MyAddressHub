...
export default function AddressesPage() {
  ...
  const [selectedAddressForPermissions, setSelectedAddressForPermissions] = useState<Address | null>(null);

  // Debug authentication
  useEffect(() => {
    console.log('AddressesPage - User:', user);
    console.log('AddressesPage - Auth token:', localStorage.getItem('accessToken'));
    console.log('AddressesPage - Refresh token:', localStorage.getItem('refreshToken'));
  }, [user]);
  ...
  const loadAddresses = async () => {
    try {
      ...
      if (error.response?.status === 401) {
        alert('Please log in to view your addresses. You will be redirected to the login page.');
  ...
  const handleCreateAddress = async (addressData: any) => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        alert('Please log in to create addresses. You will be redirected to the login page.');
        window.location.href = '/auth/login';
      } else {
  ...
  const handleUpdateAddress = async (addressData: any) => {
    ...
    try {
      ...
      const response = await addressesAPI.updateAddress(editingAddress.id, requestData);
      ...
      if (error.response?.status === 401) {
        alert('Please log in to update addresses. You will be redirected to the login page.');
        window.location.href = '/auth/login';
      } else {
        ...
        <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-300">
              This dashboard is only available for individual users. Organization users should use the organization dashboard.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600 dark:text-gray-300">
                Manage your addresses and keep them organized.
              </p>
            </div>
            <div className="flex items-center space-x-4">
              <div className="text-right">
                <p className="text-sm text-gray-500 dark:text-gray-400">Total Addresses</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalAddresses}</p>
              </div>
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Addresses</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalAddresses}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Default Address</p>
                <p className="text-lg font-semibold text-gray-900 dark:text-white">
                  {defaultAddress ? defaultAddress.addressName : 'Not set'}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="ml-4">
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">
                  {addresses.length > 0 
                    ? (() => {
                        try {
                          const date = new Date(addresses[0].updatedAt);
        ...
        {defaultAddress && (
          <div className="mb-8">
            ...
            <div className="bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-700 rounded-lg shadow-sm p-6">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                      {defaultAddress.addressName || 'Unnamed Address'} -- <span className="font-mono text-sm text-gray-500 dark:text-gray-400">{defaultAddress.id}</span>
                    </h3>
                    <button
                      onClick={() => copyToClipboard(defaultAddress.id)}
                ...
                <button
                  onClick={() => handleViewBreakdown(defaultAddress.id)}
